...
class ProxysSerializer(serializers.ModelSerializer):
    ...
    def get_today_receive(self, obj):
        order_queryset = OrderInfo.objects.filter(Q(pay_status='PAYING') |Q(pay_status='TRADE_SUCCESS') | Q(pay_status='TRADE_CLOSE'), user=obj,
    ...
    class Meta:
        model = UserProfile
        fields = ['username', 'uid', 'auth_code', 'mobile', 'notify_url', 'is_proxy', 'total_money', 'total_count_num','total_count_success_num','total_count_fail_num','total_count_paying_num','today_receive','today_count_num','today_count_success_num']
